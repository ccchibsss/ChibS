...
class HighVolumeAutoPartsCatalog:
    ...
    def determine_category_vectorized(self, name_series: pl.Series) -> pl.Series:
        ...
        for key, category in self.category_mapping.items():
            categorization_expr = categorization_expr.when(
                name_lower.str.contains(key.lower(), literal=False)
    ...
    def process_and_load_data(self, dataframes: Dict[str, pl.DataFrame]):
        ...
        parts_dfs = [
            ...
            if all(c in df.columns for c in ['artikul_norm', 'brand_norm'])
        ]
        if not parts_dfs:
            progress_bar.empty()
            st.warning("Не найдено данных для артикулов.")
            return

        parts_df = pl.concat(parts_dfs).unique(subset=['artikul_norm', 'brand_norm'], keep='first')
        file_priority = ['oe', 'barcode', 'images', 'dimensions']
        for ftype in file_priority:
            if ftype not in dataframes:
                continue
            df = dataframes[ftype]
            join_cols = [c for c in df.columns if c not in ['artikul_norm', 'brand_norm', 'artikul', 'brand']]
            if not join_cols:
                continue
        ...
        # Формирование dimensions_str
        parts_df = (parts_df
                    .with_columns([pl.col('length').cast(pl.Utf8).fill_null('').alias('_l'),
                                  pl.col('width').cast(pl.Utf8).fill_null('').alias('_w'),
                                  pl.col('height').cast(pl.Utf8).fill_null('').alias('_h')])
                    .with_columns(pl.when(pl.col('dimensions_str').is_not_null() & (pl.col('dimensions_str') != ''))
        ...
        parts_df = (parts_df
                    .with_columns([pl.col('artikul').cast(pl.Utf8).fill_null('').alias('_a'),
        ...
        parts_df = parts_df.select([pl.col(c) if c in parts_df.columns else pl.lit(None).alias(c) for c in final_columns])
    ...
    def build_export_query(self, selected_columns: Optional[List[str]] = None, include_prices: bool = True, apply_markup: bool = True) -> str:
        ...
        if include_prices:
            if apply_markup:
                ...
                price_select = f"""
                CASE
                    WHEN pr.price IS NOT NULL THEN pr.price * (1 + COALESCE(brm.markup, {global_markup}))
                    ELSE NULL
                END AS "Цена",
        ...
        # Условия исключений
        exclusion_conditions = " OR ".join([f"pd.representative_name NOT ILIKE '%{ex}%'" for ex in self.exclusion_rules if ex.strip()])
        ...
        columns_map = [
            ...
            ("Наименование", 'COALESCE(pd.representative_name, p_analog.representative_name) AS "Наименование"'),
            ("Применимость", 'COALESCE(pd.representative_applicability, p_analog.representative_applicability) AS "Применимость"'),
            ("Описание", 'CONCAT(COALESCE(p.description, ""), dt.text) AS "Описание"'),
            ("Категория товара", 'COALESCE(pd.representative_category, p_analog.representative_category) AS "Категория товара"'),
            ("Кратность", 'p.multiplicity AS "Кратность"'),
            ("Длинна", 'COALESCE(p.length, p_analog.length) AS "Длинна"'),
            ("Ширина", 'COALESCE(p.width, p_analog.width) AS "Ширина"'),
            ("Высота", 'COALESCE(p.height, p_analog.height) AS "Высота"'),
            ("Вес", 'COALESCE(p.weight, p_analog.weight) AS "Вес"'),
            ("Длинна/Ширина/Высота", """
                COALESCE(
                    NULLIF(NULLIF(p.dimensions_str, ''), 'XX'),
                    p_analog.dimensions_str
        ...
        # Выбор колонок
        selected_exprs = [expr for name, expr in columns_map if not selected_columns or name in selected_columns]
        ...
        # Упрощённый CTE-запрос
        query = f"""
        WITH DescriptionTemplate AS (SELECT $${standard_description}$$ AS text),
        BrandMarkups AS (
        ...
        AllAnalogs AS (
            SELECT 
                cr1.artikul_norm, cr1.brand_norm,
        ...
        AggregatedAnalogData AS (
            SELECT 
                cr1.artikul_norm AS artikul_norm,
                cr1.brand_norm AS brand_norm,
        ...
        FROM (
            SELECT 
                ...
                pd.oe_list, pd.representative_name, pd.representative_applicability, pd.representative_category,
                aa.analog_list,
                pa.length AS analog_length, pa.width AS analog_width, pa.height AS analog_height,
                pa.weight AS analog_weight, pa.dimensions_str AS analog_dimensions_str,
                pa.representative_name AS analog_representative_name,
                pa.representative_applicability AS analog_representative_applicability,
                pa.representative_category AS analog_representative_category,
